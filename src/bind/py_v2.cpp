...
#include <algo.h>
#include <fitting.h>
#include <planning_v2.h>
#include <depth.h>

using namespace lc;
using namespace planner_v2;
...
PYBIND11_MODULE(pylc_lib, m) {
    ...
    py::class_<Planner, std::shared_ptr<Planner>>(m, "Planner")
            .def(py::init<const CameraParameters&,
                          ...
                          const Interpolator&,
                          bool> ())
            .def_readonly("camera_angles_", &Planner::camera_angles_)
            .def("getLayoutForVis", &Planner::getVectorizedLayout)
            .def("optimizedDesignPts", &Planner::optimizedDesignPts)
            .def("randomCurtain", &Planner::randomCurtain)
            .def("randomCurtainHitProb", &Planner::randomCurtainHitProb)
            .def("loadLineSurface", &Planner::loadLineSurface)
            .def("computeVisibleRanges", &Planner::computeVisibleRanges)
            .def("visiblePoints", &Planner::visiblePoints)
            .def("computeGraphIntensities", &Planner::computeGraphIntensities)
            .def("graphIntensities", &Planner::graphIntensities)
            ;
